...
class English : Language() {
    private val messages = mapOf(
        "wahtsthename" to "${GREEN.ON}Please enter your name: ${GREEN.OFF}",
        "welcomeIntro" to "${GREEN.ON}Welcome to the Privilege Game %s!${GREEN.OFF} This game is designed to shed light on societal inequalities and the uneven distribution of opportunities. Our goal is to foster empathy towards social minorities, encourage self-reflection on your societal position, and stimulate a stance of solidarity. Let's challenge our understanding and dive into the complex dynamics of privilege. ${RED.ON}Triggerwarning: Topics such as sexual violence or racism will be mentioned.${RED.OFF}",
        "anleitungIntro" to "${GREEN.ON}The game works as follows:${GREEN.OFF} You will be asked 20+ questions. If you can answer the questions with 'yes', then press 1. If you want to answer with 'no', then press 2. For each question answered we take you a step forward or back. Additionally we added 5 NPCs. Everyone starts in the middle of the progress bar.",
...
class Polnisch : Language() {
    private val messages = mapOf(
        ...
        "wahtsthename" to "${GREEN.ON}Jak masz na imię?${GREEN.OFF}",
        )
    override fun getMessage(key: String, userName: String?): String {
        return String.format(messages[key] ?: throw IllegalArgumentException(" .. $key"), userName)
    }
...
class Arabisch : Language() {
        private val messages = mapOf(
            "welcomeIntro" to "${GREEN.ON}مرحبًا بك في لعبة الامتياز ! 1${GREEN.OFF}",
            ...
            "wahtsthename" to "${GREEN.ON}ما اسمك؟${GREEN.OFF}",
            )
    override fun getMessage(key: String, userName: String?): String {
        return String.format(messages[key] ?: throw IllegalArgumentException(" .. $key"), userName)
    }
    ...
    class Tuerkisch : Language() {
        private val messages = mapOf(
            ...
            "wahtsthename" to "${GREEN.ON} Adınız nedir?${GREEN.OFF}",
            )
        override fun getMessage(key: String, userName: String?): String {
            return String.format(messages[key] ?: throw IllegalArgumentException(" .. $key"), userName)
        }
